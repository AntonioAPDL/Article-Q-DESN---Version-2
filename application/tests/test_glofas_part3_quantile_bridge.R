...
app_glofas_part3_quantile_progress(progress_path, data.frame(iter = 1L, max_change = 1, stringsAsFactors = FALSE))
app_glofas_part3_quantile_progress(progress_path, data.frame(iter = 2L, max_change = 0.5, stringsAsFactors = FALSE))
progress <- read.csv(progress_path, check.names = FALSE)
stopifnot(nrow(progress) == 2L, identical(progress$iter, 1:2))
