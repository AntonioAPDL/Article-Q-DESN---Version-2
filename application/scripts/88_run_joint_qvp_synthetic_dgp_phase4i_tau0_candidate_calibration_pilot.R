...
out_dir <- if (nzchar(as.character(arg_value("output_dir")))) {
  ...
  app_joint_qvp_default_synthetic_dgp_phase4i_candidate_dir()
}
...
result <- app_joint_qvp_run_synthetic_dgp_phase4i_tau0_candidate_calibration_pilot(
  out_dir = out_dir,
...
cat(sprintf("Joint-QVP synthetic DGP Phase 4i tau0 candidate-pilot artifacts written to %s\n", result$out_dir))
cat(sprintf("Candidate registry rows: %s\n", nrow(result$candidate_calibration_registry)))
cat(sprintf("Candidate arms: %s\n", nrow(result$candidate_arm_grid)))
cat(sprintf("Gate: %s\n", result$phase4i_readiness_assessment$gate_status[[1L]]))
cat(sprintf("Recommendation: %s\n", result$candidate_recommendation$recommendation_status[[1L]]))
