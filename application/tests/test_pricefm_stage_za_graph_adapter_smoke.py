"""Tests for the Stage-ZA graph-adapter smoke planner."""
...
stage_za = load_script("89_prepare_pricefm_stage_za_graph_adapter_smoke.py")
...
def make_stage_z_fixture(tmp_path, *, launch_ready=False):
    ...
    args = stage_za.parser().parse_args([
        ...
        "--smoke-train-origin-limit", "25",
    ])
    return args, output, grid_config


def test_stage_za_writes_one_cell_graph_neighbor_direct_smoke(tmp_path):
    args, output, grid_config = make_stage_z_fixture(tmp_path)
    summary = stage_za.prepare(args)

    assert summary["fits_models"] is False
    assert summary["launches_models"] is False
    assert summary["writes_launch_config"] is True
    assert summary["smoke_experiments"] == 1
    assert summary["selected_region"] == "PL"
    assert summary["selected_fold"] == 3

    smoke = pd.read_csv(output / "stage_za_smoke_manifest.csv")
    assert smoke["feature_policy"].iloc[0] == "graph_neighbor_direct"
    assert smoke["train_origin_limit"].iloc[0] == 25
    assert smoke["launch_ready"].astype(bool).sum() == 0
    gates = pd.read_csv(output / "stage_za_decision_gates.csv")
    ...
    assert exp["neighbor_lag_features"] == ["price", "load"]
    assert grid["fixed"]["train_origin_limit"] == 25


def test_stage_za_rejects_launch_ready_stage_z_graph_rows(tmp_path):
    args, _, _ = make_stage_z_fixture(tmp_path, launch_ready=True)
    with pytest.raises(ValueError, match="launch-ready"):
        stage_za.prepare(args)
