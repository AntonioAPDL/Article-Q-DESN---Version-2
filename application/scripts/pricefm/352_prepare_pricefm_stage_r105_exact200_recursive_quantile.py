#!/usr/bin/env python3
"""Prepare the validation-only R105 exact-200 recursive AL/exAL campaign."""
...
OUTPUT = DATA / "launch_prep/pricefm_stage_r105_exact200_recursive_quantile_20260918"
CAMPAIGN = DATA / "campaigns/pricefm_stage_r105_exact200_recursive_quantile_20260918"
EXPECTED_REPAIR = (
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    if not 1 <= int(args.workers) <= 20:
        raise RuntimeError("R105 permits 1--20 one-core workers")
    code_root = args.code_root.resolve()
    git = git_identity(code_root)
    if not git.clean or git.head != git.upstream_head or not git.branch.startswith("work/pricefm-"):
        raise RuntimeError("R105 preparation requires a clean synchronized PriceFM task branch")
    ...
    if (
        ...
        raise RuntimeError("R105 requires the hash-pinned coherent exAL runtime")
    ...
    if len(surface_by_fold) != 3:
        raise RuntimeError("R105 requires three selected Normal-RHS validation surfaces")
    ...
    source_files = [
        ...
        Path(__file__).with_name("pricefm_recursive_quantile_marginal.py"),
        Path(__file__).with_name("353_run_pricefm_stage_r105_exact200_quantile_case.R"),
        Path(__file__).with_name("354_run_pricefm_stage_r105_exact200_quantile_case.py"),
        Path(__file__).with_name("355_closeout_pricefm_stage_r105_exact200_recursive_quantile.py"),
        Path(__file__).with_name("356_orchestrate_pricefm_stage_r105_exact200_recursive_quantile.py"),
    ...
    if any(not path.is_file() for path in source_files):
        raise FileNotFoundError("R105 source set is incomplete")
    source_records = [file_record(path, "R105_source") for path in source_files]
    ...
    for row in selected.sort_values(["region", "fold"]).itertuples(index=False):
        ...
        case_id = "r105_{}_f{}".format(region.lower(), fold)
        ...
        config = {
            ...
            "recursive_driver": "pathwise_quantile_curve_self_recursion_with_fixed_normal_neighbor_paths",
            "forecast_policies": (
                ["quantile_curve_self"]
                if len(json.loads(str(design.active_regions))) == 1
                else ["quantile_curve_self_rhs_neighbors", "quantile_curve_self_ridge_neighbors"]
            ),
            "tail_rule": "winsorize_uniforms_to_fitted_0p10_0p90_grid",
            "selection_rule": "fold1_validation_complete_family_policy_frozen_across_folds",
            "test_opened": False,
    ...
    if len(cases) != 114 or len(atoms) != 1596:
        raise RuntimeError("R105 manifest cardinality is not 114 cases / 1596 atoms")
    gates = pd.DataFrame([
        ...
        ("exact_iteration_contract", True, "min_iter=max_iter=200;patience=1"),
        ("corrected_forecast_operator", True, "pathwise_quantile_curve_self_recursion"),
    ...
    if not gates.passed.all():
        raise RuntimeError("R105 preparation gate failed")
    preview = {
        "stage": "R105",
    ...
    try:
        ...
        cases.to_csv(temporary / "pricefm_stage_r105_case_manifest.csv", index=False, quoting=csv.QUOTE_MINIMAL)
        atoms.to_csv(temporary / "pricefm_stage_r105_atom_manifest.csv", index=False, quoting=csv.QUOTE_MINIMAL)
        gates.to_csv(temporary / "pricefm_stage_r105_prep_gates.csv", index=False)
        ...
        control = {
            ...
            "workers": int(args.workers),
            "approval_token": "RUN_PRICEFM_R105_EXACT200_RECURSIVE_QUANTILE",
            "expected_cases": 114,
            "expected_atoms": 1596,
            "test_opened": False,
            **{name: False for name in BLOCKED},
        }
        write_json(temporary / "pricefm_stage_r105_launch_control.json", control)
        report = """# PriceFM Stage-R105 exact-200 recursive quantile preparation

R105 reuses the validation-selected **{panel}** Normal-RHS driver and its 114
region-fold contracts. It fits 1,596 independent VB atoms: seven AL and seven
structured exAL quantiles per case. Every atom runs exactly 200 iterations, with
50 frozen global-scale iterations followed by 150 full-objective iterations.
Training is causal teacher forced. Validation uses pathwise quantile-curve self
recursion; graph cases compare paired fixed Normal-RHS and Ridge neighbor paths.
One complete family/operator is selected per region from Fold-1 validation and
frozen across folds. Test, joint models, MCMC, registry, and article mutation
remain blocked.
""".format(panel=selected_panel)
        (temporary / "pricefm_stage_r105_launch_prep.md").write_text(report)
        outputs = {
            "cases": "pricefm_stage_r105_case_manifest.csv",
            "atoms": "pricefm_stage_r105_atom_manifest.csv",
            "gates": "pricefm_stage_r105_prep_gates.csv",
            "sources": "source_manifest.csv",
            "control": "pricefm_stage_r105_launch_control.json",
            "report": "pricefm_stage_r105_launch_prep.md",
        }
        summary = {
            **preview,
