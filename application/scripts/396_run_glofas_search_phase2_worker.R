...
done <- file.path(root, "status", paste0(job_id, ".model_done"))
...
result <- tryCatch({
  ...
  if (!identical(app_sha256_file(packet_path), as.character(row$model_packet_sha256[[1L]]))) stop("Model packet hash mismatch.", call. = FALSE)
  packet <- readRDS(packet_path)
  app_glofas_search2_validate_model_packet(packet)
  app_glofas_search2_fit_and_forecast(packet, row, args$forecast_backend)
}, error = function(e) e)
...
unlink(running)
cat(job_id, "model forecast complete and unscored\n")
