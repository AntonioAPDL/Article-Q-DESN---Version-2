...
# Launch a sharded multiseed reservoir-screening campaign for the shortlisted
# GloFAS latent-path candidates. This script does not run VB/MCMC application
# fits and does not promote manuscript outputs.
...
RUN_PREFIX="${RUN_PREFIX:-reservoir_shortlist_multiseed_full_20260525}"
N_SHARDS="${N_SHARDS:-10}"
CONFIG="${CONFIG:-application/config/glofas_latent_path_al_vb_dec25_d1n300_focused_screen.yaml}"
GRID="${GRID:-application/config/reservoir_candidate_grid_latent_path_shortlist_multiseed_20260525.csv}"
SEEDS="${SEEDS:-20260512:20260518}"
...
if [[ "$failed" -ne 0 ]]; then
  echo "At least one shortlist multiseed shard failed. Inspect application/logs/${RUN_PREFIX}_s*.log" >&2
  exit 1
fi
