#!/usr/bin/env python3
"""Build, fit, and validate one R106 region-fold independent quantile case."""
...
R_RUNNER = Path(__file__).with_name("358_run_pricefm_stage_r106_exact500_quantile_case.R")
...
def verify_prep(prep: Path) -> tuple[dict[str, Any], pd.DataFrame]:
    ...
    return summary, pd.read_csv(prep / "pricefm_stage_r106_case_manifest.csv")
...
def valid_case(path: Path, config: dict[str, Any]) -> bool:
    ...
    try:
        ...
        return (
            ...
            and bool(terminal.get("all_atoms_exact_500"))
        )
...
def run_r(config_path: Path, preflight_only: bool) -> None:
    ...
    if result.returncode:
        raise RuntimeError("R106 R fit runner failed with status {}".format(result.returncode))
...
def load_reservoir(statistics_dir: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if terminal.get("status") != "completed_causal_sufficient_statistics" or terminal.get("test_opened") is not False:
        raise RuntimeError("R106 source statistics are invalid")
    for name, record in terminal["files"].items():
        if sha256_file(statistics_dir / name) != record["sha256"]:
            raise RuntimeError("R106 source statistics changed")
...
def read_atom(config: dict[str, Any], atom: dict[str, Any]) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError("R106 atom terminal is invalid: {}".format(atom["atom_id"]))
    for record in terminal["artifacts"]:
        path = Path(record["path"])
        if not path.is_file() or sha256_file(path) != record["sha256"]:
            raise RuntimeError("R106 atom artifact changed: {}".format(path))
    p = int(terminal["p"])
    ...
    if mean.size != p or not np.all(np.isfinite(mean)) or not np.all(np.isfinite(covariance)):
        raise RuntimeError("R106 atom posterior is invalid")
...
def read_driver(
    ...
    if record.get("status") != "completed_recursive_origin" or sha256_file(path) != record.get("sha256"):
        raise RuntimeError("R106 Normal driver origin is invalid: {}".format(path))
    ...
    if stored_anchor != str(anchor):
        raise RuntimeError("R106 Normal driver anchor mismatch")
...
def score_surface(truth: np.ndarray, prediction_qnh: np.ndarray) -> dict[str, float]:
    ...
    if prediction.shape != truth.shape + (len(QUANTILES),):
        raise RuntimeError("R106 forecast surface geometry is invalid")
...
def score_case(config: dict[str, Any]) -> tuple[pd.DataFrame, list[dict[str, Any]]]:
    ...
    if fit_terminal.get("status") != "completed_recursive_quantile_fits" or fit_terminal.get("test_opened") is not False:
        raise RuntimeError("R106 fit terminal is invalid")
    ...
    if (
        ...
        raise RuntimeError("R106 Normal-RHS driver surface is invalid")
    ridge_surface = (
    ...
    if ridge_terminal.get("status") != "completed_recursive_validation_surface" or ridge_terminal.get("test_opened") is not False:
        raise RuntimeError("R106 Ridge driver surface is invalid")
    ...
    for origin_index, anchor in enumerate(context["anchors"]):
        ...
        if rhs.shape[0] != int(config["posterior_paths"]) or ridge.shape[0] != int(config["posterior_paths"]):
            raise RuntimeError("R106 Normal driver path count changed")
    ...
    for family in ("al", "exal"):
        ...
        exact = all(
            int(atom_terminals[(family, float(tau))]["iterations"]) == 500
            for tau in QUANTILES
        )
        for policy in policies:
            ...
            rows.append({
                ...
                **score_surface(truth, pred_original),
                ...
                "all_atoms_exact_500": bool(exact),
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if len(selected) != 1:
        raise RuntimeError("R106 case_id is absent or duplicated")
    row = selected.iloc[0]
    ...
    if (
        ...
        raise RuntimeError("R106 case contract identity mismatch")
    output = Path(config["output_dir"])
