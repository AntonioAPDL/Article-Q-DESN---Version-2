...
app_joint_exqdesn_phase174_build_packet <- function(
  ...
  writeLines(c(
    ...
    "This summary contains the 16 AL rows and the case-specific exAL source decision for the balanced validation design.",
    "exAL comparisons use the scale-collapsed analysis when the reported quantile functionals satisfy the stability criteria; the remaining exAL comparisons use stable MCMC summaries selected under the same design.",
    "It is the source for generated article tables.",
...
app_joint_exqdesn_phase174_relabel_generated_file <- function(path, source_line = NULL) {
  ...
  lines <- gsub(
    ...
    source_line %||% "% Generated balanced MCMC validation summary table.",
    lines, fixed = TRUE
  )
  lines <- gsub(
    ...
    paste(
      ...
      "exAL rows are included when finite-path and quantile-stability criteria are satisfied under the balanced validation design."
    ),
    lines, fixed = TRUE
  )
...
app_joint_exqdesn_phase174_stage_assets <- function(
  ...
  protocol <- data.frame(
    ...
    Value = c(
      ...
      "AL rows use the stated MCMC effort for the balanced comparison. Eleven exAL comparisons use eight chains with 24,000 iterations and 5,000 retained draws per chain; five exAL comparisons use the stable MCMC summaries selected under the same validation design.",
  ...
  gates <- data.frame(
    ...
    detail = c(
      ...
      sprintf(
        "%d exAL rows use the scale-collapsed analysis; %d comparisons use stable MCMC summaries selected under the same validation design.",
      ...
      "Generated tables are reproduced from the stated validation inputs before inclusion."
    ), stringsAsFactors = FALSE
  )
  claim_audit <- data.frame(
    ...
    evidence = c(
      ...
      "Each exAL row satisfies the finite-path and quantile-stability criteria under the balanced validation design.",
