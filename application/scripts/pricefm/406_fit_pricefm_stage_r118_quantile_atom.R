...
if (!identical(config$stage, "R118_production") ||
  ...
  stop("R118 production firewall violation", call. = FALSE)
}
for (pair in list(
...
if (!identical(manifest$status, "installed_exact_cran_exdqlm_1.1.1") ||
  ...
  stop("R118 requires exact CRAN exdqlm 1.1.1", call. = FALSE)
}
...
if (length(beta0) != p || any(!is.finite(beta0)) || !is.finite(sigma0) || sigma0 <= 0) {
  stop("R118 initializer is invalid", call. = FALSE)
}
...
external_gate <- bounded && nrow(post_release) >= as.integer(config$min_postwarmup_updates) &&
  relative_state <= 1e-3 && relative_sigma <= 1e-3 && relative_elbo <= 1e-5
if (bounded && isTRUE(fit$converged) &&
    relative_state <= 1e-3 && relative_sigma <= 1e-3 && relative_elbo <= 1e-5) {
  external_gate <- TRUE
}
...
diagnostics <- list(
  ...
  post_release_updates = nrow(post_release), bounded = bounded,
...
terminal <- c(parameters, diagnostics, list(
  status = "completed_r118_quantile_atom", stage = "R118_production",
  tag = config$tag, atom_id = config$atom_id, fold = as.integer(config$fold),
  n = n, p = p, posterior_target_sha256 = config$posterior_target_sha256,
...
if (!file.rename(temporary, output)) stop("R118 atom atomic install failed", call. = FALSE)
