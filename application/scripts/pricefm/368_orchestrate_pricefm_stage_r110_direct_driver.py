#!/usr/bin/env python3
"""Run the dependency-ordered PriceFM R110 direct-driver campaign."""
...
def physical_cpu_pool(max_workers: int) -> dict:
    ...
    if not cpus:
        raise RuntimeError("no free physical CPU is available for R110")
    return {
...
def run_parallel(tasks: list[dict], cpus: list[int], max_workers: int, status_path: Path) -> list[dict]:
    ...
    if failures:
        raise RuntimeError("R110 phase failed: " + ", ".join(row["task_id"] for row in failures))
    return results
...
def materialize_and_run(
    ...
    for spec in task_specs:
        ...
        pending.append({
            ...
            "command": [str(RSCRIPT), str(code_root / "application/scripts/pricefm/367_run_pricefm_stage_r110_direct_case.R"), "--contract", str(contract_path)],
...
def main() -> None:
    ...
    if campaign.get("test_access_authorized") is not False or campaign.get("task_count") != 45:
        raise RuntimeError("invalid R110 campaign contract")
    ...
    closeout = subprocess.run([
        sys.executable,
        str(code_root / "application/scripts/pricefm/369_closeout_pricefm_stage_r110_direct_driver.py"),
        "--campaign-root", str(root),
    ])
    if closeout.returncode != 0:
        raise RuntimeError("R110 closeout failed")
    write_json(root / "orchestrator_terminal.json", {
        "status": "completed_r110_orchestration", "stage": "R110", "task_count": 45,
        "adapter_count": 9, "selected_physical_core_count": len(cpus),
