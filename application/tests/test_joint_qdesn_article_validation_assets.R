...
stopifnot(any(grepl("Entries are averages across the joint multi-quantile simulation mechanisms", vb_model_tex, fixed = TRUE)))
stopifnot(any(grepl("approximates integrated check loss", vb_model_tex, fixed = TRUE)))
stopifnot(!any(grepl("not a single empirical data set", vb_model_tex, fixed = TRUE)))
stopifnot(!any(grepl("pinball loss", vb_model_tex, fixed = TRUE)))
