#!/usr/bin/env python3
"""Launch and resume the validation-only R105 recursive quantile campaign."""
...
WORKER = Path(__file__).with_name("354_run_pricefm_stage_r105_exact200_quantile_case.py")
CLOSEOUT = Path(__file__).with_name("355_closeout_pricefm_stage_r105_exact200_recursive_quantile.py")
APPROVAL = "RUN_PRICEFM_R105_EXACT200_RECURSIVE_QUANTILE"
THREAD_ENV = (
...
def parse_cpus(value: str) -> list[int]:
    ...
    if len(result) != len(set(result)) or any(cpu < 0 or cpu >= (os.cpu_count() or 0) for cpu in result):
        raise RuntimeError("R105 CPU list is invalid")
    return result
...
def choose_cpus(workers: int, maximum: float, explicit: str) -> tuple[list[int], dict[int, float]]:
    ...
    for cpu in candidates:
        ...
        if len(selected) == workers:
            break
    if len(selected) != workers:
        raise RuntimeError("only {} idle physical cores satisfy the R105 gate".format(len(selected)))
...
def valid_case(path: Path, case_id: str, case_contract_sha256: str) -> bool:
    ...
    try:
        ...
        return (
            artifacts_valid
            and terminal.get("all_atoms_exact_200") is True
            and len(metrics) == int(terminal["family_policy_rows"])
            and metrics.test_opened.eq(False).all()
            and metrics.all_atoms_exact_200.astype(bool).all()
        )
...
def verify_prep(prep: Path) -> tuple[dict[str, Any], pd.DataFrame]:
    ...
    cases = pd.read_csv(prep / "pricefm_stage_r105_case_manifest.csv")
    if len(cases) != 114 or cases.test_access_authorized.astype(bool).any():
        raise RuntimeError("R105 case manifest is invalid")
    return summary, cases


def run(args: argparse.Namespace) -> dict[str, Any]:
    if args.approval_token != APPROVAL:
        raise RuntimeError("R105 approval token mismatch")
    ...
    if not git.clean or git.head != git.upstream_head or git.head != summary["source_git"]["head"]:
        raise RuntimeError("R105 launch source is dirty, unsynchronized, or differs from prep")
    if shutil.disk_usage(campaign.parent).free / 1024**3 < float(args.minimum_free_gib):
        raise RuntimeError("R105 disk gate failed")
    if available_memory_gib() < float(args.minimum_available_memory_gib):
        raise RuntimeError("R105 memory gate failed")
    ...
    if failures or state["complete"] != len(cases):
        ...
        raise RuntimeError("R105 campaign has failed cases")
    ...
    if result.returncode:
        raise RuntimeError("R105 closeout failed")
    closeout_summary = DATA / "authoritative/pricefm_stage_r105_exact200_recursive_quantile_closeout_20260918/summary.json"
    ...
    campaign_terminal = {
        ...
        "atoms_complete": 1596,
        "atoms_exact_200": terminal["atoms_exact_200"],
        "atoms_formally_converged": terminal["atoms_formally_converged"],
        "atoms_numerically_eligible": terminal["atoms_numerically_eligible"],
        "selected_regions": terminal["selected_regions"],
        "unresolved_regions": terminal["unresolved_regions"],
