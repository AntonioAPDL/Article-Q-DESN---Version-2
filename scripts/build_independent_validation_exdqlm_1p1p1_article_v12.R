...
verifier <- file.path(
  ...
  "verify_independent_exdqlm_1p1p1_scoped_integration_v2.R"
)
...
if (!identical(attr(verifier_output, "status"), NULL) ||
    !any(grepl("INTEGRATION_PACKET_VERIFIED", verifier_output, fixed = TRUE))) {
  stop(paste(c("The frozen packet verifier failed:", verifier_output), collapse = "\n"),
       call. = FALSE)
}

handoff <- jsonlite::read_json(packet_path(config$packet$handoff), simplifyVector = TRUE)
git_object_file <- function(commit, relative, expected_hash, label) {
  path <- tempfile(pattern = "article-v12-baseline-")
  error_path <- tempfile(pattern = "article-v12-baseline-error-")
...
required_handoff <- c(
  ...
  package_source_commit = as.character(config$package$source_commit),
  package_tarball_sha256 = as.character(config$package$source_tarball_sha256),
  point_estimator_id = as.character(config$estimator_separation$point_exdqlm),
  interval_estimator_id = as.character(config$estimator_separation$interval_primary)
)
...
if (sum(is_exdqlm) != as.integer(expected$exdqlm_point_rows) ||
    ...
    any(point$metric_estimator_contract[is_exdqlm] !=
...
point_winner_ledger <- read.csv(
  packet_path(config$packet$point_winner_ledger), check.names = FALSE
)
interval_winner_ledger <- read.csv(
  packet_path(config$packet$interval_winner_ledger), check.names = FALSE
)
if (sum(point_winner_ledger$winner_changed) != as.integer(expected$point_winner_changes) ||
    sum(interval_winner_ledger$winner_changed) !=
      as.integer(expected$interval_winner_changes)) {
  stop("The frozen winner-change ledgers are inconsistent.", call. = FALSE)
}

# Assemble the complete 162-row diagnostic record before writing any output.
new_diagnostics <- read.csv(packet_path(config$packet$mcmc_diagnostics), check.names = FALSE)
...
for (inf in c("mcmc", "vb")) {
  ...
  for (family in families) {
    path <- article_path(sprintf(
      "tables/qdesn_validation_500obs_v12_%s_metric_intervals_%s.tex", inf, family
    ))
    writeLines(render_interval_family(inf, family), path, useBytes = TRUE)
    paths <- c(paths, path)
  }
  wrapper <- article_path(sprintf(
    "tables/qdesn_validation_500obs_v12_%s_metric_interval_tables.tex", inf
  ))
...
writeLines(c(
  ...
  expected_counts[[1L]], sum(expected_counts[3:4]), nrow(comparison)), "",
  sprintf(paste0(
...
writeLines(c(
  paste0(
    ...
    paste0(
      ...
      "entries are unchanged. The largest absolute change in an exDQLM posterior forecast-criterion ",
      "mean is 0.8464\\%; these forecast changes are not material, and the substantive ",
      "conclusions remain stable. Point estimates and posterior intervals use distinct ",
...
write_manifest <- function(path, title, artifacts, extra = character(0)) {
  ...
  writeLines(c(
    ...
    paste0("point_candidate_sha256: ", config$packet$point_candidate_sha256),
    paste0("interval_candidate_sha256: ", config$packet$interval_candidate_sha256),
    extra, "artifacts:",
...
write_manifest(
  point_table_manifest_path, "Independent single-quantile article record (v12)",
  point_artifacts,
  c("point_estimator_exdqlm: fixed_path_point_metric_chain_mean_v1", "row_count: 72")
)
point_mcmc_manifest_path <- article_path(outputs$point_mcmc_manifest)
write_manifest(
  point_mcmc_manifest_path, "Independent single-quantile MCMC article tables (v12)",
...
write_manifest(
  point_figure_manifest_path, "Independent single-quantile MCMC performance figure (v12)",
...
write_manifest(
  interval_manifest_path, "Independent posterior metric intervals (v12)",
  interval_artifacts,
  c(
...
write_manifest(
  projection_manifest_path, "Independent exdqlm 1.1.1 article projection (v12)",
  ...
  c(
    "point_rows: 72", "interval_roles: 216", "exdqlm_point_rows: 18",
    "exdqlm_interval_roles: 54", "interval_figures: 6",
    sprintf("displayed_warning_metrics: %d", displayed_warning_n)
  )
)

cat("INDEPENDENT_EXDQLM_1P1P1_ARTICLE_V12_BUILD=PASS\n")
