...
if (!identical(config$stage, "R106") || !identical(config$selection_split, "validation_only") ||
  ...
  stop("R106 case firewall violation", call. = FALSE)
}
...
if (!identical(sha256(config$runtime_manifest), config$runtime_manifest_sha256) ||
  ...
  stop("R106 runtime provenance mismatch", call. = FALSE)
}
for (record in records(config$adapters)) {
  if (!identical(sha256(record$path), record$sha256)) stop("R106 adapter hash mismatch", call. = FALSE)
...
qcfg_contract <- lapply(config$qdesn_vb, function(value) if (length(value) == 1L) unlist(value) else value)
if (!identical(as.integer(qcfg_contract$min_iter), 500L) ||
    !identical(as.integer(qcfg_contract$max_iter), 500L) ||
    !identical(as.integer(qcfg_contract$patience), 1L)) {
  stop("R106 exact-500 iteration contract mismatch", call. = FALSE)
}
old_options <- options(
  exdqlm.vb.min_iter = 500L,
  exdqlm.vb.patience = 1L
)
...
if (!identical(design_terminal$status, "completed_causal_quantile_design") ||
  ...
  stop("R106 causal design terminal is invalid", call. = FALSE)
}
for (name in names(design_terminal$files)) {
  if (!identical(sha256(file.path(config$design_dir, name)), design_terminal$files[[name]]$sha256)) {
    stop("R106 causal design hash mismatch: ", name, call. = FALSE)
  }
}
...
if (nrow(X) != length(y) || any(!is.finite(X)) || any(!is.finite(y))) {
  stop("R106 causal design is invalid", call. = FALSE)
}
...
artifact_ok <- function(path, atom_id) {
  ...
  if (is.null(value) || !identical(value$status, "completed_recursive_quantile_atom") ||
      !identical(value$stage, "R106") || !identical(value$atom_id, atom_id) ||
      !identical(as.integer(value$iterations), 500L) ||
      !identical(value$test_opened, FALSE)) return(FALSE)
...
read_parent <- function(atom) {
  ...
  if (identical(family, "normal_rhs")) {
    ...
    if (!identical(terminal$status, "completed_recursive_normal_fit") ||
      ...
      stop("R106 Normal-RHS initializer is invalid", call. = FALSE)
    }
    for (record in records(terminal$artifacts)) {
      path <- file.path(config$normal_fit_dir, record$path)
      if (!identical(sha256(path), record$sha256)) stop("R106 Normal-RHS artifact changed", call. = FALSE)
    }
  ...
  if (length(matches) != 1L) stop("R106 warm parent is missing or duplicated", call. = FALSE)
  parent <- matches[[1L]]
  if (!artifact_ok(parent$output_dir, parent$atom_id)) stop("R106 warm parent is incomplete", call. = FALSE)
...
if (preflight_only) {
  ...
  cat(jsonlite::toJSON(list(
    ...
    exact_iteration_contract = list(min_iter = 500L, max_iter = 500L, patience = 1L),
    test_opened = FALSE
...
fit_atom <- function(atom) {
  ...
  if (identical(atom$family, "exal")) {
    ...
    configured_max_iter <- as.integer(qcfg$max_iter)
  ...
  iterations <- as.integer(fit$iter %||% nrow(trace))
  if (!identical(iterations, 500L)) {
    stop("R106 fit violated the exact-500 iteration contract", call. = FALSE)
  }
  ...
  if (identical(atom$family, "exal") && nrow(trace)) {
    if (!"delta_state_relative" %in% names(trace)) {
      ...
      if (length(value) != nrow(trace)) stop("R106 relative-state trace length mismatch", call. = FALSE)
      trace$delta_state_relative <- value
    }
    if (!"delta_prediction_scaled" %in% names(trace)) {
      value <- as.numeric(deltas$prediction_scaled)
      if (length(value) != nrow(trace)) stop("R106 prediction-scale trace length mismatch", call. = FALSE)
  ...
  checks <- list(
    ...
    exact_500_iterations = identical(iterations, 500L)
  )
  updates <- 0L
  if (identical(atom$family, "exal")) {
    required <- c(
  ...
  parameters <- list(
    ...
    train_seconds = as.numeric(elapsed), iterations = iterations,
    formal_converged = isTRUE(fit$converged), structured_updates = updates,
    configured_min_iter = as.integer(qcfg$min_iter),
    configured_max_iter = as.integer(qcfg$max_iter),
    configured_patience = as.integer(qcfg$patience),
    effective_max_iter = as.integer(qcfg$max_iter),
    extended_nonconvergence_retry = FALSE,
  ...
  terminal <- list(
    ...
    stage = "R106", atom_id = atom$atom_id, case_id = config$case_id,
  ...
  if (!file.rename(temporary, atom$output_dir)) stop("R106 atom atomic install failed", call. = FALSE)
  terminal
}

dir.create(output, recursive = TRUE, showWarnings = FALSE)
write_json(list(
  status = "started", stage = "R106", case_id = config$case_id,
...
if (length(atom_terminals) != 14L || any(!vapply(atom_terminals, function(value) {
  ...
  stop("R106 case did not complete all 14 atoms", call. = FALSE)
}
summary <- list(
  status = "completed_recursive_quantile_fits",
  stage = "R106", case_id = config$case_id, region = config$region,
