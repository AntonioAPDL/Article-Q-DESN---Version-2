...
rows <- lapply(seq_len(nrow(manifest)), function(i) {
  ...
  data.frame(
    ...
    all_gates_pass = fit_ok && cold_start_ok && policy_ok,
    run_id = item$run_id[[1L]],
    stringsAsFactors = FALSE
  )
})
by_cutoff <- app_bind_rows_fill(rows)
if (!all(by_cutoff$all_gates_pass)) stop("One or more blocked-validation scientific gates failed.", call. = FALSE)
...
ranking$decision_status <- "portability_safeguard_complete_not_authoritative_forecast_evidence"
