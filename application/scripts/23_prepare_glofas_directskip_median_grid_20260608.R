#!/usr/bin/env Rscript
# Purpose: prepare a broad median-only GloFAS Q-DESN search that restores the
# direct readout input block used by the strongest previous p50 benchmark.
#
# The generated configs live under local_trackers/ and are intentionally
# ignored. This script prepares and validates launch artifacts; it does not
# itself fit models.
...
args <- app_parse_args(list(
  base_config = "local_trackers/runtime_configs/glofas_optconfirm_engine73c_memrefine16_11_d1n300_m360_a92_r95_w018_bt1em01_at3em02_skip_maxiter200_20260602.yaml",
  fallback_base_config = "tables/glofas_application_run_config__glofas_optconfirm_engine73c_memrefine16_11_d1n300_m360_a92_r95_w018_bt1em01_at3em02_skip_maxiter200_20260602.yaml",
  quantile_targets = "application/config/glofas_quantile_targets_dec25_synthesis_20260603.csv",
  out_dir = "local_trackers/runtime_configs/glofas_directskip_median_grid_20260608",
  batch_id = "glofas_directskip_median_grid_20260608",
  first_core = 30
))
...
candidate_grid <- data.frame(
  candidate_id = sprintf("D%02d", 1:24),
  search_role = c(
    "anchor_direct360",
    "direct180_same_memory",
    "direct090_same_memory",
    "direct540_same_memory",
    "reservoir240_direct180",
    "reservoir240_direct360",
    "reservoir300_direct180",
    "reservoir300_direct300",
    "reservoir420_direct180",
    "reservoir420_direct360",
    "reservoir540_direct180",
    "reservoir540_direct360",
    "slower_direct360",
    "faster_direct360",
    "lower_persistence_direct360",
    "higher_persistence_direct360",
    "weaker_drive_direct360",
    "moderate_drive_direct360",
    "stronger_drive_direct360",
    "looser_reference_direct360",
    "looser_discrepancy_direct360",
    "balanced_loose_direct360",
    "capacity400_direct360",
    "capacity500_direct180"
  ),
  D = c(rep(1L, 24)),
  n = c(
    rep("300", 22),
...
if (!file.exists(base_config_path)) {
  base_config_path <- app_path(args$fallback_base_config)
}
...
launcher <- c(
  ...
  "",
  "# Generated by application/scripts/23_prepare_glofas_directskip_median_grid_20260608.R.",
...
for (i in seq_len(nrow(candidate_grid))) {
  ...
  candidate_slug <- tolower(sprintf(
    "%s_%s_d%sn%s_m%s_dir%s_a%s_r%s_w%s_bt%s_at%s",
    cand$candidate_id,
    cand$search_role,
    cand$D,
    gsub("x", "x", cand$n),
  ...
  raw_row$notes <- sprintf("Raw GloFAS median baseline for direct-skip candidate %s (%s).", cand$candidate_id, cand$search_role)
  ...
  qdesn_row$notes <- sprintf(
    paste(
      "Q-DESN direct-skip median candidate %s (%s): D=%s, n=%s, reservoir_m=%s, direct_lag=%s,",
      "alpha=%s, rho=%s, win=%s, beta_tau0=%s, alpha_tau0=%s.",
      "Full-seven quantiles should be run only if this p50 gate beats the 0.569 reference."
    ),
    cand$candidate_id, cand$search_role, cand$D, cand$n, cand$reservoir_m,
    cand$direct_lag, cand$alpha, cand$rho, cand$win_scale_global,
    cand$beta_tau0, cand$alpha_tau0
  )
  ...
  cfg$description <- paste(
    sprintf("Broad direct-skip median-only GloFAS Q-DESN search candidate %s (%s).", cand$candidate_id, cand$search_role),
    "This is a p50 screening run; promote or full-seven relaunch only after ranking, diagnostics, and visual review."
  )
  ...
  cfg$execution$final_launch$note <- "User-approved broad direct-skip median-grid screen. This is a p50 gate, not an article-facing promotion target."
...
readme <- c(
  sprintf("# GloFAS Direct-Skip Median Grid: %s", args$batch_id),
  "",
  "This directory is generated locally and ignored by git.",
  "",
  "Purpose: run a broad median-only p50 screen across Q-DESN specifications that keep the direct readout input block enabled.",
  "",
  "Baseline to beat:",
  "- Best previous median-only p50 reference: approximately `0.5690` mean check loss from the memrefine16_11 direct-skip run.",
  "- Current promoted full-seven candidate remains the reservoir-only full-seven baseline, but this search is intended to find a stronger median/readout configuration.",
  "",
  "Search axes:",
  "- Direct readout block: `direct_lag=90,180,300,360,540`.",
  "- Reservoir memory: `m=240,300,360,420,540`.",
  "- Dynamics: `alpha=0.80,0.92,0.98`; `rho=0.90,0.95,0.98`.",
  "- Input scale: `win=0.10,0.14,0.18,0.24`.",
  "- Capacity: mostly `D=1,n=300`, plus `n=400` and `n=500` checks.",
  "- RHS prior asymmetry near `(beta_tau0, alpha_tau0)=(0.10,0.03)` plus looser variants.",
  "",
  "Decision rule:",
  "1. Rank by p50 check loss and inspect fit figures.",
  "2. Require the top candidate to beat or practically match the `0.5690` p50 reference.",
  "3. Repeat the top three candidates with additional seeds.",
  "4. Run p05/p50/p95 only for seed-stable winners.",
  "5. Run full-seven quantiles only after the p50 and three-quantile gates pass.",
  "",
  "Prepared files:",
