...
freeze_dir <- app_joint_qdesn_phase181_arg("--freeze-dir")
worker_id <- as.integer(app_joint_qdesn_phase181_arg("--worker-id"))
failure_dir <- app_joint_qdesn_phase181_arg("--failure-dir", NULL)
if (is.null(freeze_dir) || !is.finite(worker_id)) {
  stop("Usage: --freeze-dir <path> --worker-id <integer>", call. = FALSE)
}
result <- app_joint_qdesn_phase181_run_worker(
