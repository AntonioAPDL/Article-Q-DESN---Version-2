...
render_once <- function(path) {
  prefix <- tempfile(pattern = "qdesn-v13-check-")
  output <- system2(
...
check(grepl(config$outputs$mcmc_wrapper, main_text, fixed = TRUE),
      "main article uses v13 MCMC wrapper")
check(grepl(config$outputs$vb_wrapper, supplement_text, fixed = TRUE),
      "supplement uses v13 VB wrapper")
...
cat(sprintf("INDEPENDENT_DGP_ORACLE_FIGURES_V13_CHECK=PASS checks=%d\n", check_count))
