...
result <- app_joint_exqdesn_phase179_prepare_article_confirmation(
  n_vb_cores = as.integer(app_joint_exqdesn_phase176_180_arg("--vb-cores", "8")),
  force = "--force" %in% commandArgs(trailingOnly = TRUE)
)
if (identical(result$status %||% "", "no_qualified_replacements")) {
  cat("No qualified replacements; article confirmation is not required.\n")
} else {
  cat(sprintf("Phase179 article freeze: %s\nWorkers: %d\n", result$out_dir, nrow(result$plan)))
}
