...
cache_root <- arg_value("--cache-root", app_joint_exqdesn_phase164_cache_root())
out_dir <- arg_value("--output-dir", NULL)
force <- identical(tolower(arg_value("--force", "false")), "true")
result <- app_joint_exqdesn_phase176_run(
  cache_root = cache_root, out_dir = out_dir, force = force
)
cat(sprintf("Phase176 output: %s\n", result$out_dir))
print(result$classification, row.names = FALSE)
