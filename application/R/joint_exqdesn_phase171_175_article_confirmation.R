...
app_joint_exqdesn_phase174_build_packet <- function(
  ...
  readme <- file.path(out_dir, "README.md")
  writeLines(c(
    "# Balanced MCMC final summary", "",
    "This summary contains the 16 AL rows and the exAL comparison choices for the balanced evaluation design.",
    "exAL comparisons use the scale-collapsed analysis when the reported quantile functionals satisfy the stability criteria; the remaining exAL comparisons use stable MCMC summaries from the same design.",
    "It supplies the reported article tables.",
    sprintf("- Diagnostic status: `%s`", final$gate_status[[1L]]),
    sprintf("- Crossings after monotone rearrangement: `%d`", final$contract_crossing_pairs[[1L]])
  ), readme, useBytes = TRUE)
  write <- function(x, name) app_joint_qvp_write_csv(x, file.path(out_dir, name))
  paths <- c(
...
app_joint_exqdesn_phase174_relabel_generated_file <- function(path, source_line = NULL) {
  ...
  internal_comments <- grepl("^% (Generated by|Source:)", lines)
  lines <- lines[!internal_comments]
...
app_joint_exqdesn_phase174_stage_assets <- function(
  ...
  protocol <- data.frame(
    Item = c("Evaluation components", "Simulation settings", "Model comparison", "Quantile grid", "Fitting sample", "Forecast evaluation", "MCMC effort", "Reported quantile-grid summary", "Repeated-simulation comparison"),
    Value = c(
      "Variational approximations computed separately for each simulation setting provide the initial values for the MCMC analysis.",
      "Eight data-generating mechanisms with known conditional quantile paths, spanning Gaussian, Laplace, Gaussian-mixture, asymmetric-Laplace, and Student-t innovations with several dynamic structures.",
      "Joint and independently estimated quantile regressions under AL (QDESN) and exAL (exQDESN), all with the regularized horseshoe prior.",
      "0.05, 0.10, 0.25, 0.50, 0.75, 0.90, and 0.95.",
      "500 observations after the pre-specified DESN washout.",
      "Held-out rolling-origin forecasts at origins separated by 30 observations, scored at leads 1-30.",
      "AL rows use the stated MCMC effort for the complete comparison. MCMC effort varies across exAL comparisons: eleven use eight chains with 24,000 iterations and 5,000 retained draws per chain; the remaining five use simulation-setting-specific analyses satisfying the stated convergence criteria.",
      "Scores evaluate posterior quantile-grid summaries after monotone rearrangement; crossings before rearrangement are reported as a diagnostic.",
      sprintf("The repeated-simulation comparison contains %d independent VB fits.", phase153_assessment$completed_candidates[[1L]])
    ), stringsAsFactors = FALSE
  )
  gates <- data.frame(
    gate = c("Comparison inputs", "Complete comparison grid", "AL reference rows", "exAL comparison rows", "Finite reported scores", "Variational initialization", "Crossings after monotone rearrangement", "Raw crossing diagnostics", "Summary construction"),
    status = c(
    ...
    detail = c(
      ...
      "The 16 AL rows are included in the balanced MCMC comparison.",
      sprintf(
        "%d exAL rows use the scale-collapsed analysis; %d comparisons use simulation-setting-specific MCMC summaries satisfying the stated convergence criteria.",
      ...
      "Every MCMC row uses variational initialization.",
      ...
      "The table entries follow the stated evaluation inputs."
    ), stringsAsFactors = FALSE
  )
  claim_audit <- data.frame(
    ...
    evidence = c(
      ...
      "Each exAL row satisfies the finite-path and quantile-stability criteria under the balanced evaluation design.",
  ...
  gates_display <- setNames(gates, c("Check", "Result", "Detail"))
  gates_display$Result <- c(
    "Complete", "Complete", "Complete", "Complete", "Finite", "Available",
    "None", if (sum(case_summary$mcmc_forecast_raw_crossing_pairs) > 0L) "Present" else "None",
    "Consistent"
  )
  table_paths <- c(
    ...
    gate_tex = app_joint_qdesn_phase155_write_latex_table(gates_display, file.path(tables_dir, "joint_qdesn_article_validation_mcmc_balanced_gate_summary.tex"), "Diagnostic summary for the balanced MCMC multi-quantile evaluation. The checks verify complete simulation-setting--model coverage, finite reported scores, variational initialization, and separate crossing diagnostics before and after rearrangement.", "tab:joint-qdesn-article-validation-mcmc-balanced-criteria-summary", "@{}>{\\raggedright\\arraybackslash}p{0.23\\textwidth}l>{\\raggedright\\arraybackslash}p{0.60\\textwidth}@{}", size = "\\scriptsize", resize = TRUE),
    ...
    winner_tex = app_joint_qdesn_phase155_write_latex_table(winner_table, file.path(tables_dir, "joint_qdesn_article_validation_mcmc_balanced_winner_summary.tex"), "Method with the lowest displayed MCMC value within each simulation setting and criterion. Numerical rankings are descriptive when Monte Carlo uncertainty for pairwise differences is unavailable.", "tab:joint-qdesn-article-validation-mcmc-balanced-winner-summary", "@{}>{\\raggedright\\arraybackslash}p{0.20\\textwidth}>{\\raggedright\\arraybackslash}p{0.18\\textwidth}>{\\raggedright\\arraybackslash}p{0.18\\textwidth}>{\\raggedright\\arraybackslash}p{0.18\\textwidth}>{\\raggedright\\arraybackslash}p{0.18\\textwidth}@{}", size = "\\scriptsize", resize = TRUE),
    ...
    replication_tex = app_joint_qdesn_phase155_write_latex_table(replication_table, file.path(tables_dir, "joint_qdesn_article_validation_phase153_replication_summary.tex"), "Replicated VB comparison of AL and exAL forecast quantile-path MAE over 50 independent data-generating realizations for each simulation setting. Entries are the median paired difference AL minus exAL, with the percentage of replicates favoring AL in parentheses.", "tab:joint-qdesn-article-validation-phase153-replication-summary", "@{}>{\\raggedright\\arraybackslash}p{0.30\\textwidth}rrr@{}", size = "\\scriptsize")
  )
  table_paths[["scenario_tex"]] <- app_joint_qdesn_phase155_write_wrapper(table_paths[["model_tex"]], file.path(tables_dir, "joint_qdesn_article_validation_mcmc_balanced_scenario_summary.tex"), "Alternative file name for the Phase174 main table by simulation setting.")
  ...
  table_paths[["provenance_wrapper"]] <- app_joint_qdesn_phase155_write_wrapper(table_paths[c("protocol_tex", "winner_tex", "replication_tex")], file.path(tables_dir, "joint_qdesn_article_validation_provenance_tables.tex"), "Phase174 evaluation-design, criterion-specific, and replicated-VB tables.")
