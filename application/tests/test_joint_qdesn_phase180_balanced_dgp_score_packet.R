...
contrast_probe <- app_joint_qdesn_bind_rows(lapply(
  seq_len(8L),
      ...
      app_joint_qdesn_bind_rows(lapply(
        ...
        function(structure) {
          app_joint_qdesn_bind_rows(lapply(seq_len(2L), function(chain_id) {
            data.frame(
              base_scenario_id = sprintf("seed_probe_%02d", scenario_index),
              dgp_replicate_id = "replicate_1",
              variant_id = variant,
              fit_structure = structure,
              mcmc_case_id = paste(
                scenario_index, variant, structure, sep = "_"
              ),
              chain_id = chain_id,
              dgp_integrated_acrps = seq_len(4L) / 10 +
...
contrast_probe_result <-
  app_joint_qdesn_postscore_joint_independent_contrasts(
    contrast_probe, contract
  )
contrast_probe_repeat <-
  app_joint_qdesn_postscore_joint_independent_contrasts(
    contrast_probe, contract
  )
expect_true(
  nrow(contrast_probe_result$summary) == 16L &&
    identical(
      sort(unique(contrast_probe_result$draws$contrast_seed)),
      contract$contrast_pairing_seed + seq_len(16L)
    ) &&
    identical(contrast_probe_result, contrast_probe_repeat),
  "Joint-minus-independent contrasts did not use the frozen contrast seed."
)
