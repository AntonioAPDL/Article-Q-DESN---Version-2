...
cfg <- yaml::read_yaml(app_path(
  "application/config/independent_validation_dgp_oracle_figures_v14.yaml"
))
...
if (!dir.exists(validation_candidate)) {
  cat(paste0(
    "Independent DGP oracle figure v14 checks skipped: the frozen validation ",
    "worktree is unavailable; set QDESN_VALIDATION_ROOT to run this integration test.\n"
  ))
} else {
  checker <- app_path("scripts/check_independent_validation_dgp_oracle_figures_v14.R")
  checker_output <- system2("Rscript", shQuote(checker), stdout = TRUE, stderr = TRUE)
  stopifnot(is.null(attr(checker_output, "status")))
  stopifnot(any(grepl(
    "INDEPENDENT_DGP_ORACLE_FIGURES_V14_CHECK=PASS", checker_output, fixed = TRUE
  )))
  ...
  cat("Independent DGP oracle figure v14 checks passed.\n")
}
