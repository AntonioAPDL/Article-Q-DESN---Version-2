...
freeze_dir <- arg_value("--freeze-dir", app_joint_exqdesn_phase167_169_dirs()$phase169_freeze)
failure_dir <- arg_value("--failure-dir", "")
if (!is.finite(worker_id) || worker_id < 1L) {
  stop("--worker-id must be a positive integer.", call. = FALSE)
}

result <- app_joint_exqdesn_phase169_run_worker(
