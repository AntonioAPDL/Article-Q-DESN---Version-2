...
# Launch the engine-frozen 2026-05-27 application relaunch batch. Each run is
# pinned to the core recorded in the manifest and all BLAS/OpenMP thread counts
# are set to one.

MANIFEST="${1:-application/config/glofas_engine73c_relaunch_20260527_launch_manifest.csv}"
SESSION="${SESSION:-glofas_engine73c_relaunch_20260527}"
...
while IFS=, read -r _ _ _ _ _ _ _ engine_path _ _ _ _; do
...
while IFS=$'\t' read -r batch_id run_index relaunch_group source_config target_config source_model_grid target_model_grid engine_path engine_branch engine_commit run_id core; do
  window_name="$(printf 'e%02d' "$run_index")"
  log_path="${LOG_DIR}/${run_id}.log"
  cmd=$(
    printf '%q ' bash -lc "cd '$PWD' && \
export OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 VECLIB_MAXIMUM_THREADS=1 NUMEXPR_NUM_THREADS=1 && \
echo START \$(date -Is) batch_id='$batch_id' group='$relaunch_group' run_id='$run_id' config='$target_config' core='$core' engine_commit='$engine_commit' && \
