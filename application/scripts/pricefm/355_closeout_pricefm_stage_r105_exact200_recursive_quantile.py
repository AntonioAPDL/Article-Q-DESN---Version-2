#!/usr/bin/env python3
"""Close R105 and freeze one complete AL/exAL family per region."""
...
OUTPUT = DATA / "authoritative/pricefm_stage_r105_exact200_recursive_quantile_closeout_20260918"
R104_METRICS = (
...
def verify_prep(prep: Path) -> tuple[dict[str, Any], pd.DataFrame, pd.DataFrame]:
    ...
    if summary.get("status") != "prepared_validation_only_campaign" or summary.get("test_opened") is not False:
        raise RuntimeError("R105 prep is invalid")
    ...
    return (
        summary,
        pd.read_csv(prep / "pricefm_stage_r105_case_manifest.csv"),
        pd.read_csv(prep / "pricefm_stage_r105_atom_manifest.csv"),
    )
...
def select_combinations(metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    for region, group in metrics.groupby("region", sort=True):
        ...
        if (
            ...
            or not group.all_atoms_exact_200.astype(bool).all()
        ):
            raise RuntimeError("R105 family-policy surface is incomplete: {}".format(region))
        eligible = []
...
def summarize_trace(trace: pd.DataFrame) -> dict[str, Any]:
    ...
    if not required.issubset(trace.columns):
        raise RuntimeError("R105 VB trace lacks raw ELBO columns")
    iteration = trace["iter"].to_numpy(dtype=int)
    elbo = trace["elbo"].to_numpy(dtype=float)
    if len(elbo) != 200 or not np.isfinite(elbo).all():
        raise RuntimeError("R105 raw ELBO trace violates the exact-200 contract")
...
def compare_early_stop(metrics: pd.DataFrame, source: Path = R104_METRICS) -> pd.DataFrame:
    columns = [
        ...
        "early_stop_AQL", "exact200_minus_early_stop_AQL",
    ]
    ...
    result["exact200_minus_early_stop_AQL"] = (
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if len(cases) != 114 or len(atoms) != 1596:
        raise RuntimeError("R105 prep cardinality changed")
    metric_frames = []
    horizon_frames = []
    evidence = []
    terminals = []
    for row in cases.sort_values(["region", "fold"]).itertuples(index=False):
        ...
        if (
            ...
            or terminal.get("all_atoms_exact_200") is not True
        ):
            raise RuntimeError("invalid R105 case terminal: {}".format(row.case_id))
        ...
        for record in terminal["artifacts"]:
            ...
            if not path.is_file() or sha256_file(path) != record["sha256"]:
                raise RuntimeError("changed R105 case artifact: {}".format(path))
        ...
        if (
            ...
            or not metrics.all_atoms_exact_200.astype(bool).all()
        ):
            raise RuntimeError("R105 case metrics are invalid")
    ...
    if len(decisions) != 38:
        raise RuntimeError("R105 must record 38 region family-policy decisions")
    ...
    if (
        ...
        raise RuntimeError("R105 selected atom surface is incomplete")
    atom_records = []
    for row in atoms.itertuples(index=False):
        ...
        if (
            ...
            raise RuntimeError("R105 selected atom is invalid")
        for record in terminal["artifacts"]:
            path = Path(record["path"])
            if not path.is_file() or sha256_file(path) != record["sha256"]:
                raise RuntimeError("R105 selected atom artifact changed: {}".format(path))
        atom_records.append({
            **row._asdict(),
    ...
    if len(atom_status) != 1596 or not atom_status.iterations.eq(200).all():
        raise RuntimeError("R105 atom status violates the exact-200 contract")
    ...
    gates = pd.DataFrame([
        ...
        ("all_atoms_exact_200", atom_status.iterations.eq(200).all(), int(atom_status.iterations.eq(200).sum())),
    ...
    if not gates.passed.all():
        raise RuntimeError("R105 G4 closeout gate failed")
    ...
    try:
        ...
        filenames = {
            "family_policy_metrics": "pricefm_stage_r105_family_policy_validation_metrics.csv",
            "family_policy_horizon_metrics": "pricefm_stage_r105_family_policy_horizon_metrics.csv",
            "region_decisions": "pricefm_stage_r105_region_family_policy_decisions.csv",
            "atom_status": "pricefm_stage_r105_atom_status.csv",
            "selected_atoms": "pricefm_stage_r105_selected_atom_manifest.csv",
            "selected_metrics": "pricefm_stage_r105_selected_validation_metrics.csv",
            "early_stop_comparison": "pricefm_stage_r105_exact200_vs_early_stop_overlap.csv",
            "gates": "pricefm_stage_r105_g4_gates.csv",
        ...
        decision = {
            "stage": "R105",
            "status": "completed_exact200_recursive_closeout",
            "regions": 38,
            "cases": 114,
            "atoms": 1596,
            "atoms_exact_200": int(atom_status.iterations.eq(200).sum()),
            ...
            "exact200_better_overlap_rows": int((early_stop_comparison.exact200_minus_early_stop_AQL < 0).sum()),
            "selection_rule": "one_complete_family_policy_per_region_from_fold1_validation_frozen_across_folds",
            "test_opened": False,
        }
        write_json(temporary / "pricefm_stage_r105_frozen_decision.json", decision)
        report = """# PriceFM Stage-R105 recursive independent-quantile closeout

R105 completed all 114 causal region-fold cases and 1,596 independent VB
AL/exAL atoms at exactly 200 iterations. A complete family/operator combination
...
""".format(selected=len(selected_decisions), aql="NA" if global_aql is None else "{:.6f}".format(global_aql))
        (temporary / "pricefm_stage_r105_recursive_quantile_closeout.md").write_text(report)
        outputs = {
            **filenames,
            "decision": "pricefm_stage_r105_frozen_decision.json",
            "report": "pricefm_stage_r105_recursive_quantile_closeout.md",
        }
        summary = {
            **decision,
            "output_files": outputs,
            "output_sha256": {role: sha256_file(temporary / name) for role, name in outputs.items()},
            "next_gate": "manual_R105_exact200_effect_and_ELBO_review_before_any_test",
