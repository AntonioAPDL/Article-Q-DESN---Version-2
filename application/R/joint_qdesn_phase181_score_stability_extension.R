...
app_joint_qdesn_phase181_finalize <- function(
  ...
  expected_contrast_seeds <-
    freeze$contract$contrast_pairing_seed + seq_len(16L)
  if (nrow(contrast$summary) != 16L ||
      !identical(
        sort(unique(contrast$draws$contrast_seed)),
        as.integer(expected_contrast_seeds)
      )) {
    stop(
      "Phase181 must produce 16 joint/independent contrasts with the frozen seed rule.",
      call. = FALSE
    )
  }
