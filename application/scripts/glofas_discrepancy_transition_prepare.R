...
args <- app_parse_args(list(
  campaign = "application/config/glofas_discrepancy_transition_campaign_20260825.yaml",
...
if (!identical(
  ...
  "glofas_discrepancy_transition_campaign_v1"
)) {
  stop("Unsupported discrepancy-transition campaign schema.", call. = FALSE)
}
...
if (expected_count != 48L) {
  stop(sprintf("The frozen transition bridge requires 48 fits, observed %d.", expected_count), call. = FALSE)
}
if (as.integer(campaign$inference$max_iter) != 150L ||
    as.integer(campaign$execution$max_parallel) != 20L ||
    as.integer(campaign$execution$backend_threads) != 1L) {
  stop("The frozen campaign requires max_iter=150 and 20 one-thread workers.", call. = FALSE)
}
...
for (i in seq_len(nrow(candidates))) {
  ...
  for (cutoff_id in names(cutoff_assets)) {
    ...
    cfg$description <- paste(
      "Cold p50 causal discrepancy-transition bridge for", candidate_id,
      "at", asset$origin, "with FR09 geometry and priors frozen."
    )
...
provenance <- data.frame(
  field = c(
    ...
    "cutoff_registry_sha256", "runtime_manifest_sha256", "fit_count",
...
writeLines(c(
  "Causal GloFAS discrepancy-transition bridge prepared but not launched.",
  ...
  "Three v3.1 origins determine ranking; the v2.1 origin is supplemental.",
