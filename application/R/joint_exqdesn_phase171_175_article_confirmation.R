...
app_joint_exqdesn_phase174_relabel_generated_file <- function(path, source_line = NULL) {
  ...
  lines <- gsub(
    ...
    paste(
      ...
      "exAL rows are included when finite-path and quantile-stability criteria are satisfied under the balanced evaluation design."
    ),
    lines, fixed = TRUE
  )
...
app_joint_exqdesn_phase174_stage_assets <- function(
  ...
  protocol <- data.frame(
    Item = c("Evaluation components", "Synthetic mechanisms", "Model comparison", "Quantile grid", "Fit window", "Forecast protocol", "MCMC effort", "Reported quantile-grid summary", "Replicated robustness check"),
    Value = c(
      "Scenario-specific VB and structured-VB initialization followed by MCMC analysis for reported quantile-grid summaries.",
      ...
      "AL rows use the stated MCMC effort for the balanced comparison. Eleven exAL comparisons use eight chains with 24,000 iterations and 5,000 retained draws per chain; five exAL comparisons use stable MCMC summaries from the same scenario-level evaluation design.",
      "Scores evaluate posterior quantile-grid summaries after the pre-specified monotone rule; raw crossings remain diagnostics.",
      sprintf("The replicated robustness check contains %d independent VB fits.", phase153_assessment$completed_candidates[[1L]])
    ), stringsAsFactors = FALSE
  )
  gates <- data.frame(
    ...
    detail = c(
      ...
      sprintf(
        "%d exAL rows use the scale-collapsed analysis; %d comparisons use stable MCMC summaries from the same scenario-level evaluation design.",
  ...
  table_paths <- c(
    ...
    gate_tex = app_joint_qdesn_phase155_write_latex_table(gates_display, file.path(tables_dir, "joint_qdesn_article_validation_mcmc_balanced_gate_summary.tex"), "Diagnostic summary for the balanced MCMC multi-quantile evaluation. The checks verify complete scenario--model coverage, finite reported scores, variational initialization, and separation of raw and post-rearrangement crossing diagnostics.", "tab:joint-qdesn-article-validation-mcmc-balanced-criteria-summary", "@{}>{\\raggedright\\arraybackslash}p{0.23\\textwidth}l>{\\raggedright\\arraybackslash}p{0.60\\textwidth}@{}", size = "\\scriptsize", resize = TRUE),
    ...
    winner_tex = app_joint_qdesn_phase155_write_latex_table(winner_table, file.path(tables_dir, "joint_qdesn_article_validation_mcmc_balanced_winner_summary.tex"), "Method with the lowest displayed MCMC value within each scenario and metric. Numerical rankings are descriptive when Monte Carlo margins are unresolved.", "tab:joint-qdesn-article-validation-mcmc-balanced-winner-summary", "@{}>{\\raggedright\\arraybackslash}p{0.20\\textwidth}>{\\raggedright\\arraybackslash}p{0.18\\textwidth}>{\\raggedright\\arraybackslash}p{0.18\\textwidth}>{\\raggedright\\arraybackslash}p{0.18\\textwidth}>{\\raggedright\\arraybackslash}p{0.18\\textwidth}@{}", size = "\\scriptsize", resize = TRUE),
