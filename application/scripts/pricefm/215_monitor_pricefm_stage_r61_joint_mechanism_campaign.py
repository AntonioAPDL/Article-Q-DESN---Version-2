#!/usr/bin/env python3
"""Monitor R61 and perform validation-only postfit repair and closeout."""
...
OUTPUT = DATA / "authoritative/pricefm_stage_r61_joint_mechanism_monitor_20260826"
REPAIR_OUTPUT = DATA / "authoritative/pricefm_stage_r61_joint_mechanism_postfit_20260826"
CLOSEOUT = DATA / "authoritative/pricefm_stage_r61_joint_mechanism_closeout_20260826"
...
def health(manifest: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    for row in manifest.itertuples(index=False):
        ...
        postfit_complete = (
            ...
            and (model / "source_manifest.csv").is_file()
        )
        rows.append({
            "case_id": row.case_id,
            ...
            "fit_artifact_complete": fit_artifact_complete,
...
def run(args: argparse.Namespace) -> dict:
    ...
    if len(manifest) != args.expected_runs:
        raise RuntimeError("R61 monitor manifest count changed")
    if "test_access_authorized" not in manifest.columns:
        raise RuntimeError("R61 monitor manifest lacks the test firewall")
    if manifest.test_access_authorized.astype(bool).any():
        raise RuntimeError("R61 monitor refuses a manifest that authorizes test access")
    args.output_dir.mkdir(parents=True, exist_ok=True)
    repair_script = Path(__file__).with_name("205_repair_pricefm_stage_r57_joint_vb_postfit.py")
    closeout_script = Path(__file__).with_name(
        "214_closeout_pricefm_stage_r61_joint_mechanism_campaign.py"
    )
    while True:
        frame, counts = health(manifest)
        frame.to_csv(args.output_dir / "pricefm_stage_r61_joint_mechanism_health.csv", index=False)
        status = "running"
        ...
        if counts["test_opened"]:
            ...
            frame, counts = health(manifest)
            frame.to_csv(
                args.output_dir / "pricefm_stage_r61_joint_mechanism_health.csv", index=False
            )
            if (
                counts["postfit_complete"] == args.expected_runs
                and not repair_summary.get("repair_failures", 0)
            ):
        ...
        result = {
            ...
            **counts,
            ...
            "models_refit_by_monitor": 0,
            "test_opened": counts["test_opened"],
