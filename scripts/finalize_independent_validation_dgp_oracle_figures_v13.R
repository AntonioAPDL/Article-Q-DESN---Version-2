...
config_path <- file.path(
  repo_root, "application", "config", "independent_validation_dgp_oracle_figures_v13.yaml"
)
...
render_signature <- function(path) {
  ...
  for (i in seq_len(2L)) {
    prefix <- tempfile(pattern = paste0("qdesn-v13-render-", i, "-"))
    output <- system2(
      "pdftocairo",
...
builder_path <- file.path(repo_root, "scripts", "build_independent_validation_dgp_oracle_figures_v13.R")
checker_path <- file.path(repo_root, "scripts", "check_independent_validation_dgp_oracle_figures_v13.R")
pipeline_path <- file.path(repo_root, "scripts", "run_independent_validation_dgp_oracle_figures_v13.sh")
wrapper_paths <- c(
...
cat(sprintf(
  "INDEPENDENT_DGP_ORACLE_FIGURES_V13_FINALIZED figures=%d dpi=300\n",
  length(figure_paths)
))
