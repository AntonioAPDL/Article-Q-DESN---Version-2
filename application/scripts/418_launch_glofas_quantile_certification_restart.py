...
"""Dependency- and certification-gated launcher for GloFAS restart jobs."""
...
def verify_contract(repo: Path, root: Path) -> tuple[list[dict[str, object]], dict[str, object]]:
    manifest_path = root / "configs/certification_job_manifest.json"
    contract_path = root / "configs/certification_contract.json"
    jobs = json.loads(manifest_path.read_text())
    contract = json.loads(contract_path.read_text())
    if sha256(manifest_path) != contract["manifest_sha256"]:
        raise RuntimeError("Certification manifest hash does not match the frozen contract")
    ...
    if len(jobs) != 18 or sum(job["action"] == "fit" for job in jobs) != 9:
        raise RuntimeError("Certification manifest must contain nine fits and nine forecasts")
...
def health(root: Path, jobs: list[dict[str, object]], active: dict[str, subprocess.Popen]) -> dict[str, object]:
    ...
    counts = {key: sum(value == key for value in states.values()) for key in ("completed", "running", "failed", "blocked", "pending")}
...
def main() -> None:
    ...
    parser.add_argument("--session-label", default="glofas_quantile_certification_restart_20260921")
