...
if (!identical(config$stage, "R119_production") ||
    !identical(config$family, "al") ||
    !identical(config$variant, "extended") ||
    !identical(config$readout, "extended_all_layers") ||
  ...
  stop("R119 production firewall violation", call. = FALSE)
}
for (pair in list(
  ...
  if (!identical(sha256(pair[[1L]]), pair[[2L]])) stop("R119 source hash mismatch", call. = FALSE)
}
...
if (!identical(manifest$status, "installed_exact_cran_exdqlm_1.1.1") ||
  ...
  stop("R119 requires exact CRAN exdqlm 1.1.1", call. = FALSE)
}
...
feature_names <- unlist(design$feature_names, use.names = FALSE)
if (length(feature_names) != p || !identical(feature_names[[1L]], "intercept") ||
    !any(startsWith(feature_names, "input::")) ||
    !all(vapply(1:3, function(layer) any(startsWith(feature_names, paste0("layer", layer, "::"))), logical(1L)))) {
  stop("R119 extended readout design identity failed", call. = FALSE)
}
...
if (identical(config$parent_type, "normal_rhs")) {
  sigma0 <- sqrt(
    as.numeric(parent_terminal$omega_rate) /
      max(as.numeric(parent_terminal$omega_shape) - 1, 1e-8)
  )
} else {
...
if (length(beta0) != p || any(!is.finite(beta0)) || !is.finite(sigma0) || sigma0 <= 0) {
  stop("R119 initializer is invalid", call. = FALSE)
}
...
fit <- r67_fit_quantile(
  config$cran_library, X, y, as.numeric(config$tau), "al", rhs, qcfg,
  profile = profile, init = list(beta = beta0, sigma = sigma0),
...
bounded <- finite_core && sigma < 100
external_gate <- bounded && nrow(trace) >= 35L && relative_state <= 1e-3 &&
...
parameters <- list(
  family = "al", tau = as.numeric(config$tau), tau0 = as.numeric(config$tau0),
...
diagnostics <- list(
  ...
  post_release_updates = nrow(trace), bounded = bounded,
  finite_core = finite_core, formal_converged = isTRUE(fit$converged),
  external_gate_passed = external_gate
)
...
terminal <- c(parameters, diagnostics, list(
  status = "completed_r119_quantile_atom", stage = "R119_production",
  tag = config$tag, atom_id = config$atom_id, variant = "extended",
  readout = "extended_all_layers", fold = as.integer(config$fold), n = n, p = p,
  posterior_target_sha256 = config$posterior_target_sha256,
  initializer_source = normalizePath(config$parent_dir, mustWork = TRUE),
  initializer_changes_prior = FALSE, sigmagam_freeze_warmup_iters = 0L,
...
if (!file.rename(temporary, output)) stop("R119 atom atomic install failed", call. = FALSE)
