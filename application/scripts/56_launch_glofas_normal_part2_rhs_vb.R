...
args <- app_parse_args(list(
  runtime_root = "local_trackers/runtime_configs/glofas_normal_part2_rhs_top10_vb_20260903",
  workers = "20",
  session_label = "",
  poll_seconds = "300"
))
...
worker_log <- file.path(runtime_root, "logs", "part2_rhs_worker_pool.log")
closeout_log <- file.path(runtime_root, "logs", "part2_rhs_scheduler_closeout.log")
watch_log <- file.path(runtime_root, "logs", "part2_rhs_watch.log")
scheduler_script <- file.path(runtime_root, "configs", "run_part2_rhs_worker_pool.sh")
watch_script <- file.path(runtime_root, "configs", "watch_part2_rhs_health.sh")
...
worker_script <- paste(
  ...
  sprintf("echo \"[$(date)] rebuilding %d Part2 ridge warm starts with %d workers\" >> \"$worker_log\"", length(warm_ids), warm_workers),
  sprintf(
    "cat \"$warm_ids\" | xargs -r -P %d -I{} sh -c 'Rscript application/scripts/53_run_glofas_normal_part2_rhs_warm_start_worker.R --runtime_root \"$0\" --candidate_id \"$1\" >> \"$0/logs/part2_rhs_worker_pool.log\" 2>&1 || true' \"$runtime_root\" {}",
    warm_workers
  ),
  sprintf("echo \"[$(date)] launching %d Part2 Normal RHS/VB fits\" >> \"$worker_log\"", length(rhs_ids)),
  sprintf(
    "cat \"$rhs_ids\" | xargs -r -P %d -I{} sh -c 'Rscript application/scripts/54_run_glofas_normal_part2_rhs_vb_worker.R --runtime_root \"$0\" --rhs_candidate_id \"$1\" >> \"$0/logs/part2_rhs_worker_pool.log\" 2>&1 || true' \"$runtime_root\" {}",
    workers
  ),
  "Rscript application/scripts/55_check_glofas_normal_part2_rhs_vb.R --runtime_root \"$runtime_root\" >> \"$closeout_log\" 2>&1",
  "echo \"[$(date)] Part2 Normal RHS/VB scheduler finished\" >> \"$worker_log\"",
  sep = "\n"
)
watch_script_text <- paste(
  ...
  sprintf("cd %s", shQuote(repo_root)),
  sprintf(
    "while true; do date; Rscript application/scripts/55_check_glofas_normal_part2_rhs_vb.R --runtime_root %s; sleep %d; done >> %s 2>&1",
