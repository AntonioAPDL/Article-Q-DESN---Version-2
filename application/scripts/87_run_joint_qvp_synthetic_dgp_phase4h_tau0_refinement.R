...
args <- app_parse_args(list(
  ...
  output_dir = "",
  tier = "targeted",
  tau0_grid = "0.25,0.20,0.15,0.10,0.075,0.05",
...
out_dir <- if (nzchar(as.character(arg_value("output_dir")))) {
  ...
  app_joint_qvp_default_synthetic_dgp_phase4h_screen_dir()
}
...
reference_screen_id <- if (nzchar(as.character(arg_value("reference_screen_id")))) {
  as.character(arg_value("reference_screen_id"))
} else {
  NULL
}

result <- app_joint_qvp_run_synthetic_dgp_phase4h_tau0_refinement(
  out_dir = out_dir,
  targeted_registry_path = targeted_registry_path,
  tau0_grid = parse_number_grid(arg_value("tau0_grid")),
  include_secondary_grid = app_as_bool(arg_value("include_secondary_grid")),
...
cat(sprintf("Joint-QVP synthetic DGP Phase 4h tau0-refinement artifacts written to %s\n", result$out_dir))
cat(sprintf("Targeted registry rows: %s\n", nrow(result$targeted_registry)))
cat(sprintf("Screen rows: %s\n", nrow(result$tau0_refinement_grid)))
cat("Screen statuses:\n")
print(table(result$tau0_refinement_candidate_ranking$screen_status))
cat(sprintf("Recommendation: %s\n", result$tau0_refinement_recommendation$recommendation_status[[1L]]))
