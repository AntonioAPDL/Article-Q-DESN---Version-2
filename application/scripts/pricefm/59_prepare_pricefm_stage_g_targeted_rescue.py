#!/usr/bin/env python3
"""Prepare a Stage-G targeted median rescue grid.

Stage G starts from the current authoritative PriceFM quantile decision
registry. It prepares a validation-clean median-only grid for region/folds
where the local DESN/Q-DESN decision is still close to, or worse than, cached
fold-aligned PriceFM Phase-I.

The generated grid is deliberately tiered:

* priority 0: close losses and the largest fallbacks;
* priority 1: moderate fallbacks;
* priority 2: mild fallbacks.

Selection remains validation AQL only. Test and PriceFM metrics are copied into
metadata as audit fields and must not be used to choose median candidates.
"""
...
DEFAULT_MEDIAN_REGISTRY = (
    ...
    "pricefm_stage_f_graph_median_rescue_promoted_registry_20260621/"
    "patched_selection_registry.csv"
)
DEFAULT_DECISION_REGISTRY = (
    "application/data_local/pricefm/authoritative/"
    "pricefm_authoritative_quantile_decisions_stage_f_20260621/"
    "authoritative_quantile_decision_registry.csv"
)
DEFAULT_OUTPUT_GRID = (
    "application/data_local/pricefm/configs/"
    "pricefm_desn_experiment_grid_stage_g_targeted_median_rescue_20260621.yaml"
)
DEFAULT_GENERATED_ROOT = (
    "application/data_local/pricefm/experiment_grids/"
    "pricefm_stage_g_targeted_median_rescue_20260621"
)
DEFAULT_RUN_ROOT = (
...
DEFAULT_SUMMARY_DIR = (
    "application/data_local/pricefm/authoritative/"
    "pricefm_stage_g_targeted_median_rescue_plan_20260621"
)
...
def parser():
    ...
    p.add_argument("--grid-id", default="pricefm_stage_g_targeted_median_rescue_20260621")
    ...
    p.add_argument("--priority0-delta-abs", type=float, default=1.25)
    p.add_argument("--priority0-delta-rel", type=float, default=0.15)
    p.add_argument("--priority1-delta-rel", type=float, default=0.08)
    p.add_argument("--max-variants-priority0", type=int, default=18)
    p.add_argument("--max-variants-priority1", type=int, default=10)
    p.add_argument("--max-variants-priority2", type=int, default=6)
    p.add_argument("--candidate-source", default="stage_g_targeted_median_rescue_20260621")
...
def d1_units(units, factor=1.35, cap=240):
    base = int(units[-1] if units else 120)
    return [int(min(cap, max(80, round(base * factor / 20) * 20)))]


def d2_units(units):
    total = max(120, int(sum(units) if units else 160))
...
def read_median_registry(path):
    ...
    if frame.duplicated(["region", "fold"]).any():
        ...
        raise ValueError("median registry duplicate rows: {}".format(
...
def read_decision_registry(path):
    ...
    if frame.duplicated(["region", "fold"]).any():
        dup = frame[frame.duplicated(["region", "fold"], keep=False)]
        raise ValueError("decision registry duplicate rows: {}".format(
            dup[["region", "fold"]].drop_duplicates().to_dict("records")
        ))
    return frame


def stage_g_priority(row, args):
    ...
    if decision == "stage_c_local_close_to_pricefm":
        return 0
    if delta_abs >= float(args.priority0_delta_abs) or delta_rel >= float(args.priority0_delta_rel):
        return 0
    if delta_rel >= float(args.priority1_delta_rel):
        return 1
    return 2
...
def rescue_scope(decisions, args):
    ...
    if out.empty:
        raise ValueError("No Stage-G rescue rows remain after decision filtering.")
    out["stage_g_rescue_reason"] = out.apply(lambda row: rescue_reason(row, args), axis=1)
    out["stage_g_rescue_priority"] = out.apply(lambda row: stage_g_priority(row, args), axis=1)
    out["selection_rule"] = "validation_AQL_median_only_test_pricefm_audit"
    out["test_metrics_role"] = "audit_only"
    return out.sort_values(["stage_g_rescue_priority", "delta_abs", "region", "fold"], ascending=[True, False, True, True]).reset_index(drop=True)
...
def merge_median(scope, median):
    ...
    if not missing.empty:
        raise ValueError("median registry missing Stage-G rows: {}".format(
            missing.to_dict("records")
        ))
    return out.sort_values(["stage_g_rescue_priority", "delta_abs", "region", "fold"], ascending=[True, False, True, True]).reset_index(drop=True)


def variant_spec(row, tag, metadata, updates, candidate_source):
    ...
    spec = {
        "id": "stageg_{}_f{}_{}".format(slug(region), fold, tag),
        "stage": "stage_g_targeted_median_rescue",
        "priority": int(row["stage_g_rescue_priority"]),
        "regions": [region],
        "folds": [fold],
        "quantile": 0.50,
        "target_label": "stage_g_targeted_median_rescue_validation",
        ...
        "stage_g_rescue_reason": str(row["stage_g_rescue_reason"]),
        ...
        "rationale": (
            "Stage-G targeted median rescue: region={}, fold={}, tag={}, "
...
def dedupe_variants(variants):
    ...
    for spec in variants:
        signature = json.dumps({
            "feature_policy": spec.get("feature_policy"),
            "graph_degree": spec.get("graph_degree", ""),
            "lag_window": spec.get("lag_window"),
            "depth": spec.get("depth"),
            "units": spec.get("units"),
            "alpha": spec.get("alpha"),
            "rho": spec.get("rho"),
            "input_scale": spec.get("input_scale"),
            "projection_scale": spec.get("projection_scale"),
        }, sort_keys=True)
...
def candidate_variants(row, args):
    ...
    def add(tag, metadata, updates=None):
        variants.append(variant_spec(row, tag, metadata, updates or {}, args.candidate_source))

    # A target-only anchor is retained as a guard against graph-specific damage
    # on rows that already entered the registry with graph features. Add it
    # before graph perturbations so the max-variant cap cannot prune it away.
    if str(row_value(row, "feature_policy", "")) == "graph_khop":
    ...
    # Graph-khop remains the main rescue mechanism because Stage-F converted
    # several difficult rows to local wins with graph inputs.
    for degree in [1, 2]:
        meta = graph_metadata(degree)
        prefix = "graphd{}".format(degree)
        add("{}_base".format(prefix), meta)
        add("{}_input_lo".format(prefix), meta, {"input_scale": round_control(clamp(input_scale * 0.75, 0.05, 1.0))})
        add("{}_input_hi".format(prefix), meta, {"input_scale": round_control(clamp(input_scale * 1.35, 0.05, 1.0))})
        add("{}_alpha_lo".format(prefix), meta, {"alpha": round_control(clamp(alpha - 0.10, 0.10, 0.90))})
        add("{}_alpha_hi".format(prefix), meta, {"alpha": round_control(clamp(alpha + 0.10, 0.10, 0.90))})
        add("{}_rho_lo".format(prefix), meta, {"rho": round_control(clamp(rho - 0.08, 0.10, 1.25))})
        add("{}_rho_hi".format(prefix), meta, {"rho": round_control(clamp(rho + 0.08, 0.10, 1.25))})
        if priority == 0:
            for candidate_lag in [48, 72, 144]:
                if candidate_lag != lag:
                    add("{}_lag{}".format(prefix, candidate_lag), meta, {"lag_window": int(candidate_lag)})
            add("{}_d1_units_hi".format(prefix), meta, {"units": d1_units(units)})
...
def build_grid(template_payload, merged, args):
    ...
    grid["purpose"] = (
        "Stage-G targeted median rescue generated from the authoritative "
        "Stage-F seven-quantile decision registry. Selection is median "
    ...
    grid["fixed"]["stage_g_target_registry"] = config_path_value(args.authoritative_decision_registry_csv)
    grid["fixed"]["stage_g_median_registry"] = config_path_value(args.median_registry_csv)
    grid.setdefault("launch", {})
    priorities = sorted({int(exp["priority"]) for exp in grid["experiments"]})
    grid["launch"]["stage_g_targeted_median_rescue"] = {
        ...
        "note": (
            ...
            "the largest fallbacks; priorities 1 and 2 are optional expansion."
        ),
    }
    return payload
...
def write_summary(summary_dir, args, scope, merged, payload):
    ...
    scope.to_csv(out / "stage_g_targeted_rescue_scope.csv", index=False)
    merged.to_csv(out / "stage_g_targeted_rescue_source_rows.csv", index=False)
    ...
    counts = {
        ...
        "by_rescue_reason": {
            str(k): int(v)
            for k, v in scope["stage_g_rescue_reason"].value_counts().to_dict().items()
        },
    }
    summary = {
        "status": "completed",
        ...
        "recommended_priority0_launch": (
            ...
            "--grid-config {} --priorities 0 --experiment-jobs 10 --cell-jobs 1 "
            "--build-windows true --resume true --dry-run false"
        ).format(config_path_value(args.output_grid_config)),
    }
    write_json(out / "summary.json", summary)
    with open(out / "stage_g_targeted_rescue_plan_report.md", "w") as f:
        f.write("# PriceFM Stage-G Targeted Median Rescue Plan\n\n")
        f.write("This generated plan targets authoritative Stage-F close/fallback rows.\n\n")
        f.write("Grid config: `{}`\n\n".format(config_path_value(args.output_grid_config)))
        ...
        f.write("- Test and PriceFM metrics remain audit fields only.\n")
        f.write("- Seed robustness is required before any quantile promotion.\n")
        f.write("- Priority 0 is the recommended first launch tier.\n")
