...
space <- app_glofas_median_campaign_space(campaign, anchor = anchor)
...
app_write_csv(data.frame(
  ...
  value = c(
    ...
    app_sha256_file(anchor$fit_object_path),
    nrow(app_glofas_median_screen_candidate_manifest(space)),
...
status <- system2(Sys.which("Rscript"), prepare_args)
