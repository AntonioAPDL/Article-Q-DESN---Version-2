#!/usr/bin/env python3
"""Health report for the causal GloFAS discrepancy-transition campaign."""
...
def main():
    ...
    states = {
        ...
        for row in read_csv(root / "scheduler_state.csv")
    }
    rows = [
    ...
    print("GloFAS causal discrepancy-transition bridge")
    print(
    ...
    print(f"root={root}")
    ranking = read_csv(root / "tables" / "transition_candidate_ranking.csv")
    if ranking:
        leader = ranking[0]
        print(
            "finalized=true "
    ...
    if args.details:
        print("candidate_id\tcutoff\tstatus\tstage\tcpu\tlog_age_min")
        for row in rows:
            print(
                f"{row['candidate_id']}\t{row['cutoff_id']}\t{row['status']}\t"
