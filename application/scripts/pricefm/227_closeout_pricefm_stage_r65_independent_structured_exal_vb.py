#!/usr/bin/env python3
"""Close out R65 with validation-only whole-bundle selection."""
...
OUTPUT = DATA / "authoritative/pricefm_stage_r65_independent_structured_exal_vb_closeout_20260829"
...
def run(args: argparse.Namespace) -> dict:
    ...
    if len(manifest) != args.expected_cases or len(components) != args.expected_cases * 7:
        raise RuntimeError("R65 manifest/component coverage is incomplete")
    if status.case_id.nunique() != args.expected_cases or status.status.isin(["failed", "launcher_exception"]).any():
        raise RuntimeError("R65 launch has not completed cleanly")
    ...
    for row in manifest.sort_values(["region", "fold"]).itertuples(index=False):
        ...
        required = [
            ...
            model / "r65_component_status.csv",
            model / "r65_case_fit_summary.json",
        ]
        if not all(path.is_file() for path in required):
            raise RuntimeError(f"R65 case is incomplete: {row.case_id}")
        metric = pd.read_csv(model / "metric_summary.csv")
        component_status = pd.read_csv(model / "r65_component_status.csv")
        convergence_pass = (
        ...
        synthesis_rows.extend([
            ...
            {
                "case_id": row.case_id, "region": row.region, "fold": int(row.fold),
                "family": "structured_exal", "raw_validation_AQL": new_exal,
        ...
        selected_family = "structured_exal" if structured_eligible else f"legacy_{legacy_selected}"
        ...
        case_rows.append({
            ...
            "new_structured_exal_validation_AQL": new_exal,
            ...
            "decision": "freeze_structured_exal_validation_winner" if structured_eligible else "retain_r62_authority",
    ...
    queue = decisions[decisions.decision.eq("freeze_structured_exal_validation_winner")].copy()
    decisions.to_csv(output / "pricefm_stage_r65_case_decisions.csv", index=False)
    parity.to_csv(output / "pricefm_stage_r65_al_parity_components.csv", index=False)
    synthesis.to_csv(output / "pricefm_stage_r65_family_synthesis_diagnostics.csv", index=False)
    queue.to_csv(output / "pricefm_stage_r65_frozen_test_audit_queue.csv", index=False)
    gates = pd.DataFrame([
    ...
    if not gates.passed.all():
        raise RuntimeError(f"R65 closeout gates failed: {gates.loc[~gates.passed].to_dict('records')}")
    gates.to_csv(output / "pricefm_stage_r65_closeout_gates.csv", index=False)
    fixed = [args.manifest.resolve(), args.component_ledger.resolve(), args.launch_status.resolve(), Path(__file__).resolve()]
    ...
    summary = {
        "status": "completed_validation_only_r65_closeout",
        "cases": len(decisions),
    ...
    (output / "pricefm_stage_r65_closeout_report.md").write_text(
        "# PriceFM Stage-R65 validation-only closeout\n\n"
        f"R65 closed {len(decisions)} independent seven-quantile region/fold cases. "
