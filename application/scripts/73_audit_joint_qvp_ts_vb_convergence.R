...
out_dir <- if (length(args) >= 1L && nzchar(args[[1L]])) {
  ...
  app_path("application/cache/joint_qvp_ts_vb_convergence_audit_20260702")
}
...
result <- app_joint_qvp_run_ts_vb_convergence_audit(
  out_dir = out_dir,
  calibration_dir = calibration_dir
)
cat(sprintf("Joint-QVP time-series VB convergence audit artifacts written to %s\n", result$out_dir))
cat(sprintf("Artifact manifest: %s\n", result$paths[["artifact_manifest"]]))
cat(sprintf("Audit report: %s\n", result$paths[["audit_report"]]))
cat("Resolution counts:\n")
print(table(result$case_resolution_summary$resolution_status))
