...
if (file.exists(terminal_path)) {
  ...
  if (identical(terminal$status, "completed_r110_case") &&
...
if (any(!is.finite(X_train_all)) || any(!is.finite(y_train_all))) {
  stop("R110 train design must be finite", call. = FALSE)
}
...
if (identical(contract$prior_type, "rhs_ns") && any(!vapply(fits, function(x) isTRUE(x$converged), logical(1)))) {
  stop("R110 RHS fit did not satisfy the frozen convergence criterion", call. = FALSE)
}
...
if (any(!is.finite(q_eval))) stop("nonfinite R110 quantile prediction", call. = FALSE)
...
if (identical(contract$phase, "outer_validation")) {
  ...
  if (n_paths != 500L) stop("R110 final prediction requires exactly 500 paths", call. = FALSE)
  if (identical(contract$readout, "shared")) {
  ...
  if (any(!is.finite(paths))) stop("nonfinite R110 predictive path", call. = FALSE)
}
...
terminal <- list(
  status = "completed_r110_case", stage = "R110", task_id = contract$task_id,
...
if (!file.rename(tmp, output_dir)) stop("failed to install R110 task output", call. = FALSE)
