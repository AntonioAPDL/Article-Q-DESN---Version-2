...
if (!identical(config$stage, "R105") || !identical(config$selection_split, "validation_only") ||
  ...
  stop("R105 case firewall violation", call. = FALSE)
}
...
if (!identical(sha256(config$runtime_manifest), config$runtime_manifest_sha256) ||
  ...
  stop("R105 runtime provenance mismatch", call. = FALSE)
}
for (record in records(config$adapters)) {
  if (!identical(sha256(record$path), record$sha256)) stop("R105 adapter hash mismatch", call. = FALSE)
...
if (!identical(as.integer(qcfg_contract$min_iter), 200L) ||
    !identical(as.integer(qcfg_contract$max_iter), 200L) ||
    !identical(as.integer(qcfg_contract$patience), 1L)) {
  stop("R105 exact-200 iteration contract mismatch", call. = FALSE)
}
old_options <- options(
  exdqlm.vb.min_iter = 200L,
...
if (!identical(design_terminal$status, "completed_causal_quantile_design") ||
  ...
  stop("R105 causal design terminal is invalid", call. = FALSE)
}
for (name in names(design_terminal$files)) {
  if (!identical(sha256(file.path(config$design_dir, name)), design_terminal$files[[name]]$sha256)) {
    stop("R105 causal design hash mismatch: ", name, call. = FALSE)
  }
}
...
if (nrow(X) != length(y) || any(!is.finite(X)) || any(!is.finite(y))) {
  stop("R105 causal design is invalid", call. = FALSE)
}
...
artifact_ok <- function(path, atom_id) {
  ...
  if (is.null(value) || !identical(value$status, "completed_recursive_quantile_atom") ||
      !identical(value$stage, "R105") || !identical(value$atom_id, atom_id) ||
      !identical(as.integer(value$iterations), 200L) ||
...
read_parent <- function(atom) {
  ...
  if (identical(family, "normal_rhs")) {
    ...
    if (!identical(terminal$status, "completed_recursive_normal_fit") ||
      ...
      stop("R105 Normal-RHS initializer is invalid", call. = FALSE)
    }
    for (record in records(terminal$artifacts)) {
      path <- file.path(config$normal_fit_dir, record$path)
      if (!identical(sha256(path), record$sha256)) stop("R105 Normal-RHS artifact changed", call. = FALSE)
    }
  ...
  if (length(matches) != 1L) stop("R105 warm parent is missing or duplicated", call. = FALSE)
  parent <- matches[[1L]]
  if (!artifact_ok(parent$output_dir, parent$atom_id)) stop("R105 warm parent is incomplete", call. = FALSE)
...
if (preflight_only) {
  ...
  cat(jsonlite::toJSON(list(
    ...
    exact_iteration_contract = list(min_iter = 200L, max_iter = 200L, patience = 1L),
    test_opened = FALSE
...
fit_atom <- function(atom) {
  ...
  if (!identical(iterations, 200L)) {
    stop("R105 fit violated the exact-200 iteration contract", call. = FALSE)
  }
  ...
  if (identical(atom$family, "exal") && nrow(trace)) {
    if (!"delta_state_relative" %in% names(trace)) {
      ...
      if (length(value) != nrow(trace)) stop("R105 relative-state trace length mismatch", call. = FALSE)
      trace$delta_state_relative <- value
    }
    if (!"delta_prediction_scaled" %in% names(trace)) {
      value <- as.numeric(deltas$prediction_scaled)
      if (length(value) != nrow(trace)) stop("R105 prediction-scale trace length mismatch", call. = FALSE)
  ...
  checks <- list(
    ...
    exact_200_iterations = identical(iterations, 200L)
  )
  updates <- 0L
  if (identical(atom$family, "exal")) {
    required <- c(
  ...
  terminal <- list(
    ...
    stage = "R105", atom_id = atom$atom_id, case_id = config$case_id,
  ...
  if (!file.rename(temporary, atom$output_dir)) stop("R105 atom atomic install failed", call. = FALSE)
  terminal
}

dir.create(output, recursive = TRUE, showWarnings = FALSE)
write_json(list(
  status = "started", stage = "R105", case_id = config$case_id,
...
if (length(atom_terminals) != 14L || any(!vapply(atom_terminals, function(value) {
  ...
  stop("R105 case did not complete all 14 atoms", call. = FALSE)
}
summary <- list(
  status = "completed_recursive_quantile_fits",
  stage = "R105", case_id = config$case_id, region = config$region,
