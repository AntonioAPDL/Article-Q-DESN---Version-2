...
worker_id <- as.integer(value("--worker-id"))
freeze_dir <- value("--freeze-dir", app_joint_exqdesn_phase176_dirs()$phase177_freeze)
failure_dir <- value("--failure-dir", "")
if (!is.finite(worker_id) || worker_id < 1L) stop("--worker-id is required.", call. = FALSE)
...
cat(sprintf("Phase177 worker %d: %s\n", result$worker_id, result$status))
