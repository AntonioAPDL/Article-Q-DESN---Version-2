#!/usr/bin/env python3
"""Prepare the validation-only R106 exact-500 recursive AL/exAL campaign."""
...
OUTPUT = DATA / "launch_prep/pricefm_stage_r106_exact500_recursive_quantile_20260918"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    if not 1 <= int(args.workers) <= 20:
        raise RuntimeError("R106 permits 1--20 one-core workers")
    code_root = args.code_root.resolve()
    git = git_identity(code_root)
    if not git.clean or git.head != git.upstream_head or not git.branch.startswith("work/pricefm-"):
        raise RuntimeError("R106 preparation requires a clean synchronized PriceFM task branch")
    ...
    if (
        ...
        raise RuntimeError("R106 requires the hash-pinned coherent exAL runtime")
    ...
    if len(surface_by_fold) != 3:
        raise RuntimeError("R106 requires three selected Normal-RHS validation surfaces")
    ...
    source_files = [
        ...
        Path(__file__).with_name("358_run_pricefm_stage_r106_exact500_quantile_case.R"),
        Path(__file__).with_name("359_run_pricefm_stage_r106_exact500_quantile_case.py"),
        Path(__file__).with_name("360_closeout_pricefm_stage_r106_exact500_recursive_quantile.py"),
        Path(__file__).with_name("361_orchestrate_pricefm_stage_r106_exact500_recursive_quantile.py"),
    ...
    if any(not path.is_file() for path in source_files):
        raise FileNotFoundError("R106 source set is incomplete")
    source_records = [file_record(path, "R106_source") for path in source_files]
    ...
    for row in selected.sort_values(["region", "fold"]).itertuples(index=False):
        ...
        case_id = "r106_{}_f{}".format(region.lower(), fold)
    ...
    if len(cases) != 114 or len(atoms) != 1596:
        raise RuntimeError("R106 manifest cardinality is not 114 cases / 1596 atoms")
    gates = pd.DataFrame([
        ...
        ("exact_iteration_contract", True, "min_iter=max_iter=500;patience=1"),
    ...
    if not gates.passed.all():
        raise RuntimeError("R106 preparation gate failed")
    preview = {
        "stage": "R106",
    ...
    try:
        ...
        cases.to_csv(temporary / "pricefm_stage_r106_case_manifest.csv", index=False, quoting=csv.QUOTE_MINIMAL)
        atoms.to_csv(temporary / "pricefm_stage_r106_atom_manifest.csv", index=False, quoting=csv.QUOTE_MINIMAL)
        gates.to_csv(temporary / "pricefm_stage_r106_prep_gates.csv", index=False)
        ...
        control = {
            ...
            "workers": int(args.workers),
            "approval_token": "RUN_PRICEFM_R106_EXACT500_RECURSIVE_QUANTILE",
            "expected_cases": 114,
            "expected_atoms": 1596,
            "test_opened": False,
            **{name: False for name in BLOCKED},
        }
        write_json(temporary / "pricefm_stage_r106_launch_control.json", control)
        report = """# PriceFM Stage-R106 exact-500 recursive quantile preparation

R106 reuses the validation-selected **{panel}** Normal-RHS driver and its 114
region-fold contracts. It fits 1,596 independent VB atoms: seven AL and seven
structured exAL quantiles per case. Every atom runs exactly 500 iterations, with
50 frozen global-scale iterations followed by 450 full-objective iterations.
...
""".format(panel=selected_panel)
        (temporary / "pricefm_stage_r106_launch_prep.md").write_text(report)
        outputs = {
            "cases": "pricefm_stage_r106_case_manifest.csv",
            "atoms": "pricefm_stage_r106_atom_manifest.csv",
            "gates": "pricefm_stage_r106_prep_gates.csv",
            "sources": "source_manifest.csv",
            "control": "pricefm_stage_r106_launch_control.json",
            "report": "pricefm_stage_r106_launch_prep.md",
        }
        summary = {
            **preview,
