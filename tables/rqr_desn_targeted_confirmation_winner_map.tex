% Generated from package-side RQR-DESN targeted-confirmation audit winner_map.csv.
\begin{table}[!htbp]
\centering
\scriptsize
\resizebox{\textwidth}{!}{%
...
\toprule
Stage & Mechanism & $c$ & Selected readout & Interval score & Coverage & $\Delta$ score & Cal. \\
\midrule
...
\caption{RQR--DESN targeted-confirmation winners. The coverage target is $c$. Selected readouts are reported as design/prior/learning-rate. $\Delta$ score is the paired interval-score difference relative to the empirical train-interval baseline, so negative values favor the RQR--DESN readout. Calibration classes use the package-side audit thresholds, with green indicating absolute coverage error at most 0.05 and yellow at most 0.075.}
\label{tab:rqr-desn-targeted-confirmation}
\end{table}
