...
args <- app_parse_args(list(
  ...
  source_fit_path = "", source_fit_sha256 = "",
  source_cumulative_trace_path = "", source_cumulative_trace_sha256 = "",
  expected_source_iterations = "400", design_cache = "", design_cache_sha256 = "",
...
expected_source_iterations <- as.integer(args$expected_source_iterations)
...
if (!is.finite(expected_source_iterations) || expected_source_iterations < 200L ||
    expected_source_iterations %% 200L != 0L) {
  stop("Production continuation requires a positive source count divisible by 200.", call. = FALSE)
}
...
validate_source_trace <- function(path, expected_iterations) {
  trace <- utils::read.csv(path, stringsAsFactors = FALSE, check.names = FALSE)
  observed <- as.integer(trace$global_iter %||% integer())
  if (nrow(trace) != expected_iterations ||
      !identical(observed, seq_len(expected_iterations))) {
    stop("Source cumulative trace is not an exact 1:N sequence.", call. = FALSE)
  }
  trace
}
...
run_continuation <- function() {
  ...
  source_trace_path <- truth_sha(
    args$source_cumulative_trace_path, args$source_cumulative_trace_sha256,
    "source cumulative trace"
  )
  ...
  source_trace <- validate_source_trace(source_trace_path, expected_source_iterations)
  cache <- readRDS(cache_path)
  tau <- parse_tau(args$tau)
  if (app_glofas_restart_source_iterations(source_fit) != expected_source_iterations) {
    stop("Source fit completed-iteration count does not match the continuation contract.", call. = FALSE)
  }
  if (!isTRUE(source_fit$checkpoint_state$complete_local_state)) {
    stop("Source fit is not a complete checkpoint.", call. = FALSE)
  }
  ...
  if (part %in% c("part1", "part2")) {
    ...
    if (!identical(restart$restart_kind, "exact_local_state_available")) {
      stop("Production continuation requires exact local state.", call. = FALSE)
    }
    ...
    if (!identical(restart$restart_kind, "exact_local_state_available")) {
      stop("Production Part 3 continuation requires exact local state.", call. = FALSE)
    }
  ...
  continuation_iterations <- as.integer(args$max_iter)
  ...
  certificate_row <- app_glofas_restart_certificate_row(
    ...
    expected_segment_iterations = continuation_iterations,
    expected_source_iterations = expected_source_iterations
  )
  expected_cumulative <- expected_source_iterations + continuation_iterations
  if (nrow(cumulative_trace) != expected_cumulative ||
      !identical(as.integer(cumulative_trace$global_iter), seq_len(expected_cumulative))) {
    stop("Combined continuation trace is not an exact 1:N sequence.", call. = FALSE)
  }
  fit$certification_continuation <- list(
    source_fit_path = source_fit_path,
    source_fit_sha256 = as.character(args$source_fit_sha256[[1L]]),
    source_cumulative_trace_path = source_trace_path,
    source_cumulative_trace_sha256 = as.character(args$source_cumulative_trace_sha256[[1L]]),
    ...
    source_iterations = expected_source_iterations,
    segment_iterations = continuation_iterations,
    cumulative_iterations = expected_cumulative,
  ...
  contract <- data.frame(
    ...
    source_state_sha256 = restart$source_state_sha256,
    restart_kind = restart$restart_kind,
    design_cache = cache_path,
    design_cache_sha256 = as.character(args$design_cache_sha256[[1L]]),
    prior_source_sha256 = prior_identity$source$sha256,
    prior_continuation_sha256 = prior_identity$restart$sha256,
    ...
    cumulative_trace_sha256 = app_sha256_file(cumulative_path),
...
ok <- tryCatch({ run_continuation(); TRUE }, error = function(e) {
...
message(sprintf("Completed certification continuation: %s", job_id))
